"""vLLM-Metal adapter implementation."""
...
class VLLMMetalAdapter(BackendAdapter):
    """Backend adapter that translates Switchyard requests into vLLM runtime calls."""
    ...
    def __init__(
        ...
        *,
        ...
        if model_config.backend_type is not BackendType.VLLM_METAL:
            msg = (
                "VLLMMetalAdapter requires a LocalModelConfig with "
                "backend_type='vllm_metal'"
            )
            raise ValueError(msg)

        self.model_config = model_config
        self.name = f"vllm-metal:{model_config.alias}"
        self.backend_type = BackendType.VLLM_METAL
        self._runtime = runtime or VLLMMetalChatRuntime(model_config)
